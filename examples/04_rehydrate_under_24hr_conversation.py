            ...
            while True:
                ...
                try:
                    ...
                    for act_obj in activity_set.activities:
                        ...
                        if act_obj.type == "message":
                            logger.info(f"← {sender_name}: {summary}")
                        else:
...
async def example_usage(user_input: str, previous_conversation_id: str) -> None:
    ...
    try:
        ...
        previous_conversation: Optional[Transcript] = get_transcript_from_conversation_id(
            conversation_id=previous_conversation_id
        )
        initial_event_activity_for_previous_conversation: Activity = Activity(
            type="event",
            name="setPreviousConversationsContext",
            value=previous_conversation.model_dump(),
            var_from=ChannelAccount(id=USER_ID),
            locale="en-AU",
            channel_id="directline",
        )
        await send_activity(conversations_api_token, conv_id, initial_event_activity_for_previous_conversation, sent_activities_log)
...
if __name__ == "__main__":
    # Ask for input of conversation_id (A3jp5dOHccH6glPTbyhPBo-au)
    conversation_id = input("Enter the conversation ID: ")
    # Potentially ask, (What have we talked about?)
    ...
    if not USER_ID:
        ...
        asyncio.run(example_usage(user_input=user_input, previous_conversation_id=conversation_id))
